...
object Adts {
  ...
  def getAt(list: List[Int], index: Int): Option[Int] = index match {
    case _ if list.indices.contains(index) => Some(list(index))
    case _ => None
  }

  // примените функцию из пункта (a) здесь, не изменяйте сигнатуру 
  def testGetNth(list: List[Int], n: Int): Option[Int] = getAt(list, n)

  // b) Напишите функцию, увеличивающую число в два раза.

  def double(n: Option[Int]): Option[Int] = n match {
    case s: Some[Int] => Some(s.get * 2)
    case _ => None
  }
